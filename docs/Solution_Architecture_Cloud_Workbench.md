...
| Cost of the platform itself | Token and infrastructure cost tracked per request, and attributed to the vertical that caused it | A FinOps tool should measure its own cost, and charge it back the way it charges back everything else |
| Bounded generation spend | A token budget per persona and per vertical, enforced in the API layer before a request reaches a model | An unbounded per-request cost is the one thing this platform can't defend. See ADR-010 |
...
**ADR-010: Cap generation spend per persona and vertical in the API layer, cache prompt prefixes, and never cache by meaning**
- *Context*: Every other cost on this platform is bounded by something: warehouses have resource monitors, automated actions have `policy_run_caps`, and infrastructure is fixed. Generation is the exception. A single vertical can run up an arbitrary bill with a loop of expensive questions, and a FinOps platform that can't govern its own AI spend is hard to defend. Caching is the obvious lever to pull alongside a cap, but the design has already rejected an embedding index (ADR-006) and a retrieval cache at launch (ADR-003).
- *Decision*: Three parts, all in the API layer where personas are already enforced (Build Specification §7):
  1. **Token budgets**, per persona and per vertical, daily and monthly, checked before a request reaches a model and enforced by `policy_token_caps`. Crossing a budget refuses the request with the reason and the reset time, the same way a run cap sends an action back to advisory. The Platform persona gets a larger budget than a Vertical one, because the FinOps team's work is the platform's purpose.
  2. **Request rate limits** per persona and per API key, so one client can't exhaust a budget in a minute.
  3. **Prompt caching at the provider**, by putting stable content (persona rules, metric catalog, schema) at the front of the prompt and the question at the end. Azure OpenAI caches an identical prefix of at least 1,024 tokens automatically and discounts those input tokens, reporting them as `cached_tokens` ([References.md](References.md) R29). No component to run.
- *Alternatives considered*: **Semantic caching**, rejected. It answers a question judged similar to an earlier one, over data that changes daily and is scoped per vertical; the failure modes are serving yesterday's number as today's, or one vertical's number to another. It also reintroduces the embedding model ADR-006 removed. **Exact-match answer caching** keyed on resolved parameters and the gold data version, not rejected but deferred to ADR-003's trigger: it is safe because the key changes when the data reloads, and it is worth adding only if measurement shows repeat questions matter. **Budget alerts without enforcement**, rejected; an alert that arrives after the spend is a report, not a control. **No cap at all, relying on the provider's quota**, rejected; a provider quota protects the provider's capacity, not the organization's budget, and it can't tell one vertical from another.
- *Consequences*: Generation spend is bounded per tenant and predictable per month, and it is attributable, so the platform's own AI cost is charged back like any other cost (§2.3). A vertical that exhausts its budget is refused until reset, which is a visible failure rather than a silent bill; the budget is a configuration value, so raising it is a decision someone makes on purpose. Cost per task becomes a comparable number in the provider bake-off (ADR-004). Prompt caching only pays off while the stable prefix stays byte-identical, so prompt edits invalidate it, which is one more reason prompt changes go through the eval gate.
