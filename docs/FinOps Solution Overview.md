...
| Opportunity | Solution Component | Where It's Detailed | Status |
|---|---|---|---|
| Confidence + evidence-scored reconciliation, confidence + impact-based routing | Bill Verification model, extending Core Intelligence's pattern (ADR-M4) | Bill Verification (entire) | Detailed |
| PO auto-drafting from verified line items | — | — | **Planned** — a distinct, downstream capability, not part of Bill Verification itself; see Sub-Document Index's "PO Auto-Draft" |
...
| Detailed Build Specification: FinOps Intelligence Platform                                                       | 1–3, 5                                         | [Platform_Build_Specification.md](Platform_Build_Specification.md)                                 | Companion to the five docs above — table names, job names, function signatures, policy names at build-ready specificity. Bill Verification's gold-layer tables (§3) are included; a dedicated Bill Verification build section (matching §5/§6's pattern) is not yet added — see Review Status                                                                                                                                                                                                                                                               |
...
| PO Auto-Draft: GenAI-Assisted Purchase Order Generation                                                          | 5 (extension, downstream of Bill Verification) | Not yet created                                                                                              | A distinct capability from Bill Verification, not part of it — Bill Verification's confidence-scored output is a prerequisite input, not the same pipeline. Sketched components: a drafting template/skill, the verified line-item data, GenAI to produce a proposed PO, a verifier of the produced output, a human approval step, a database for output/approvals/verifications, and (if possible) a means to post the approved PO to the relevant system. Most naturally built as a Cloud Workbench-adjacent tool once that agentic infrastructure exists |
