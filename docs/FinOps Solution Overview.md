...
The solution is organized into five phases, each building on the data and governance foundation established by the ones before it. Phase 1 through Phase 4 already have detailed sub-documents; Phase 5 is scoped here for the first time.
...
| Bill verification | Reconciles invoiced line items against contracted rates and expected usage, producing a confidence score that routes high-confidence/low-impact items to auto-clear and everything else to a human reviewer | Bill Verification (entire); Build Specification §3 |
| Cloud Workbench Expansion | Adds push (signals embedded in IDP/CMP/the real Internal Assistant, staged by signal value) and pull (a vertical-facing what-if workbench aggregating those same tools plus cost/APM/reference data, reusing Core Intelligence's models for projections) to Phase 5's Cloud Workbench, submitting what-if results into Governed Automation's identical risk-tiered pipeline | Cloud Workbench Expansion (entire); Governed Automation §3.5 |
...
| Anonymized cross-vertical benchmarking, exposed to verticals | Cloud Workbench Expansion FR5 | Cloud Workbench Expansion §3.4 | Detailed — extends the existing metric/tool to Vertical sessions, no redesign |
| Push: FinOps signals embedded in IDP/CMP/Internal Assistant | Cloud Workbench Expansion FR1 | Cloud Workbench Expansion §3.2 | Detailed — staged by signal value (cloud optimization → IDP, Kubernetes → CMP, both → Internal Assistant once its API surface is confirmed) |
| Pull: vertical self-serve workbench + what-if analysis | Cloud Workbench Expansion FR2–FR4 | Cloud Workbench Expansion §3.3 | Detailed — reuses Core Intelligence's models for projections; a liked scenario can be self-initiated as a proposed action, scoped to this surface (ADR-4 there) |
...
| Horizontal/vertical "contract" | Contract entity + `check_contract_exists` gate | Governed Automation: The Horizontal/Vertical Contract (entire); Governed Automation §3.3–3.4 | Detailed — CAB/change-window system of record still unconfirmed (Contract doc ADR-4) |
| Policy schema & engine | Policy-as-code | Build Specification §6 | Detailed — consent/contract fields now modeled (`contracts` table, Build Specification §6) |
...
| Opportunity | Solution Component | Where It's Detailed | Status |
|---|---|---|---|
| Confidence + evidence-scored reconciliation, confidence + impact-based routing | Bill Verification model, extending Core Intelligence's pattern (ADR-M4) | Bill Verification (entire); Build Specification §10 | Detailed |
| PO auto-drafting from verified line items | GenAI drafting + deterministic verification, always human-approved | PO Auto-Draft (entire) | Detailed — ERP/procurement posting system unconfirmed (PO Auto-Draft ADR-4) |
...
| **3 — Governed Automation** | Risk-tiered action layer split into two deliberately separate components: an **Orchestrator** (Temporal) running the durable workflow — sequencing, opt-out timers, approval waits, retries, rollback — and a **Guardrail Engine** (OPA policy-as-code) the Orchestrator calls out to for every LOW/MEDIUM/HIGH classification, neither one deciding both what to do and whether it's safe. Extended with the horizontal/vertical contract and a disposition-SLA escalation path | Depends on Phase 2's recommendations existing before there's anything to automate; also needs to be proven and mature before Phase 5 adds a second, agent-driven proposal source on top of it |
...
| **5 — Cloud Workbench** | The bounded, explicitly-graphed agentic self-serve interface (chatbot) — a typed pydantic-graph state machine, not an open-ended autonomous loop, distinct from Phase 3's workflow Orchestrator — plus its planned push/pull/what-if expansion | Deliberately last, not alongside Phase 2: prove the data foundation, ML pipeline, and Governed Automation through simpler surfaces first, before investing in the more complex agentic layer — see ADR-M1 |

---

## Migration & Cutover Sequence

The phase table above groups components by architectural dependency; this section is the granular, step-by-step rollout order layered on top of it — how the organization actually cuts over from the current state to this platform, one validated increment at a time rather than a single big-bang switch. Each step gates on an explicit pass/fail validation before the next begins.

1. **Data pipelines (go-forward, current period)** — ingest and process current-period data through bronze/silver/gold (Data Foundations §4.1–4.3). Historical backfill is deliberately deferred to step 8, so current-period value lands before waiting on full history.
2. **Data A/B testing** — validate go-forward processed data against the existing store, with explicit gates: entity resolution (`dq_check_apm_id_present`) at or above the ~nearly all baseline, plus `dq_check_duplicate_line_item`, `dq_check_currency_valid`, `dq_check_null_resource_id` (Build Specification §2).
3. **Semantic layer** — establish and validate Semantic Views (Data Foundations §4.4) for accuracy.
4. **Repoint existing Power BI reports (current-period scope)** — fork reports, then repoint to the semantic/gold layer.
5. **Power BI A/B testing** — compare repointed vs. existing report output.
6. **Report generation A/B testing** — same comparison for reports generated outside Power BI (automated or manual).
7. **Report generation go-live** — following regression and UAT sign-off.
8. **Historical backfill** — one-time load of historical data from the legacy SQL Server store into bronze/silver/gold; validated with step 2's same gates, applied retroactively.
9. **Extend repointed Power BI reports to historical data** — extend step 4's forked reports to the backfilled range; re-run Power BI A/B testing scoped to the new historical range specifically, not a re-check of step 5.
10. **Knowledge graph (Neo4j) sync** — prerequisite: a lightweight ontology artifact should exist first (Data Foundations §4.4 flags this as not yet formalized), so Neo4j's schema implements that artifact rather than standing in for it by default. Then develop, test, and validate the gold-to-Neo4j sync job (Data Foundations ADR-004); validation = graph traversal answers match direct gold-table queries, for the overlap of questions both can answer.
11. **Self-Serve API (`cost-intelligence-api`)** — develop and standard-test, scoped per Data Foundations / Self-Serve Foundations; any bill-verification or model-interaction functionality belongs to those respective docs as designed, not enumerated here.
12–14. **Anomaly Detection** — model through SDLC with wiring for identification/notification, scoped to the FinOps team only at first (12); A/B test against existing process output (13); official go-live in blue/green fashion, expanding to verticals over time (14).
15–17. **Rightsizing** — the same three-step pattern (model/wiring → A/B test → blue/green go-live) as Anomaly Detection.
18–20. **RI Planning** — the same three-step pattern again.
21. **Cloud Workbench** — develop and test; Platform (FinOps/platform team) rollout first, dogfooded against questions the team already knows the answer to, then expanded to Vertical access — the same shadow-mode pattern as steps 12–20, not a new one.
22. **Security/Compliance sign-off (Cloud Workbench Vertical rollout)** — required before expanding beyond Platform use, tied to the PII-redaction policy and governance NFRs already named in the Cloud Workbench doc.
23. **Legacy system decommission** — explicit parallel-run duration and decommission trigger, once every consuming surface above (reports, API, models, Cloud Workbench) is live and validated.

Governed Automation's rollout (Phase 3) is intentionally not sequenced above — see Open Items to Validate.

---

## Migration Communications

Communication is planned as its own cross-cutting workstream alongside the sequence above, not an afterthought folded into individual steps: general advisories throughout the cutover, plus targeted notice whenever a specific step impacts downstream consumers.

- **Cadence**: general advisories at the start of each phase and at each go-live (steps 7, 14, 17, 20, 22–23), so affected teams know where the cutover stands without having to track the sequence themselves.
- **Targeted notice — step 9 (extend repointed Power BI reports to historical data)**: historical values appearing or changing in reports as the backfilled range comes online is the kind of change that looks like a data-quality regression if consumers aren't warned first.
- **Targeted notice — steps 21–22 (Cloud Workbench Vertical rollout)**: verticals gaining a new conversational interface, and the security/compliance sign-off gating it, both warrant direct notice to the verticals affected, not just a general advisory.
- **Targeted notice — step 23 (legacy system decommission)**: the parallel-run window and its end date should be communicated well ahead of the actual decommission, not announced only once cutover is already complete.
...
| Solution Architecture: Governed Automation                                                                       | 3                                              | [Solution_Architecture_Governed_Automation.md](Solution_Architecture_Governed_Automation.md)       | The action layer's risk-tiered automation (2c)                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| Solution Architecture: Governed Automation — The Horizontal/Vertical Contract                                    | 3 (extension)                                  | [Solution_Architecture_Governed_Automation_Contract.md](Solution_Architecture_Governed_Automation_Contract.md) | 2c's contract artifact, consent schema, lifecycle, signing mechanism, and the `check_contract_exists` gate — extends the Governed Automation doc rather than a wholly separate document. CAB/change-window system of record unconfirmed (ADR-4 there) |
| Solution Architecture: Bill Verification as Exception-Based Review                                               | 4                                              | [Solution_Architecture_Bill_Verification.md](Solution_Architecture_Bill_Verification.md)           | Dedicated architectural treatment of 2d — schema, pipeline, confidence/routing logic, ADR — extending the MLOps pattern rather than inventing a new one                                                                                                                                                                                                                                                                                                                                                                                                     |
...
| Solution Architecture: PO Auto-Draft — GenAI-Assisted Purchase Order Generation | 5 (extension, downstream of Bill Verification) | [Solution_Architecture_PO_Auto_Draft.md](Solution_Architecture_PO_Auto_Draft.md) | Drafts a PO from Bill Verification cleared line items, verified deterministically, always human-approved — never auto-posts; Build Specification §11 |
| Detailed Build Specification: FinOps Intelligence Platform                                                       | 1–5                                         | [Platform_Build_Specification.md](Platform_Build_Specification.md)                                 | Companion to the five docs above — table names, job names, function signatures, policy names at build-ready specificity. Bill Verification now has its own dedicated build section (§10), matching §5/§6's pattern for Cloud Workbench/Governed Automation                                                                                                                                                                                                                                                               |
| Solution Architecture: Cloud Workbench Expansion — Push/Pull Channels & What-If Analysis | 5 (extension) | [Solution_Architecture_Cloud_Workbench_Expansion.md](Solution_Architecture_Cloud_Workbench_Expansion.md) | 2b's dashboard, push-embedding, pull-workbench, what-if analysis, and benchmarking items, extending the existing Cloud Workbench doc rather than a wholly separate document; Build Specification §12. Internal Assistant's API surface remains unconfirmed (ADR-1 there); push stages it last regardless |
...
- Data retention window (Data Foundations §4.3): whether the 36-month silver/gold archive figure is total-from-ingestion or additional on top of the initial 18 months live — flagged there as unconfirmed, this document assumes the former.
- Governed Automation rollout (Phase 3): the transition from advisory-only to actually acting on infrastructure (risk-tiered auto-execution, opt-out windows, approval queues), the Terraform-state/IaC-managed data ingestion it depends on, and the corresponding security/compliance sign-off are deliberately left out of the Migration & Cutover Sequence above, pending a dedicated cutover conversation for Phase 3. The architecture itself — Governed Automation and its Contract extension — is already designed; only its rollout ordering into that sequence remains open.
