...
| Application Portfolio Management (APM) tool | The organization's application metadata system (owner, technical contact, classification). Its APM ID is tagged on nearly all cloud resources and is already used by Security and GRC | A much stronger entity-resolution signal than cloud tags alone: an authoritative, organization-level key. Feeds the ontology (§4.4). Ingestion and data-quality checks are in the Build Specification |
